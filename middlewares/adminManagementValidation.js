...
const ADMIN_ROLE_OPTIONS = new Set(["MainAdmin", "SalesAdmin", "SalesStaff", "HRManager", "AgentAdmin"]);
...
const validateAgentSettingsQuery = (req, res, next) => {
  ...
  if (role !== undefined && !ADMIN_ROLE_OPTIONS.has(String(role).trim())) {
    return res.status(400).json({ message: "role must be one of: MainAdmin, SalesAdmin, SalesStaff, HRManager, AgentAdmin" });
  }
